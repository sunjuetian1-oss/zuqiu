...
export default function AdminPage() {
  ...
  return (
    <div style={{ maxWidth: 600, margin: '0 auto', padding: '20px 16px', minHeight: '100vh' }}>
      <div style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 28, color: '#e8b84b', marginBottom: 4 }}>
        ADMIN · 分析生成
      </div>
      <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', marginBottom: 20 }}>
        每场比赛点击生成一次，所有用户共享同一份分析
      </div>
        ...
        <div key={m.id} style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', marginTop: 2 }}>
              {m.group}组 · {formatDate(m.date)} {m.time}
            </div>
          </div>
          <button
            ...
            style={{
              padding: '6px 14px',
              background: results[m.id] === 'ok' ? 'rgba(29,158,117,0.2)' : results[m.id] === 'error' ? 'rgba(200,50,50,0.2)' : 'rgba(232,184,75,0.15)',
              border: `1px solid ${results[m.id] === 'ok' ? '#1D9E75' : results[m.id] === 'error' ? '#e24b4a' : 'rgba(232,184,75,0.4)'}`,
              borderRadius: 6,
              color: results[m.id] === 'ok' ? '#1D9E75' : results[m.id] === 'error' ? '#e24b4a' : '#e8b84b',
              fontSize: 12, cursor: loading === m.id ? 'not-allowed' : 'pointer',
